...
export function useValue<V extends string>(
  needles: V[],
  defaultValue: NoInfer<V>
): CurrentAppliedType<V> {
  const { styleState } = useExtensionState();

  return useMemo(
    () => ({
      current:
        (styleState.currentTailwindStyles.find((s) =>
          needles.some((c) => s.endsWith(c))
        ) as V) ?? defaultValue,
      applied:
        (styleState.appliedTailwindStyles.find((s) =>
          needles.some((c) => s.endsWith(c))
        ) as V) ?? defaultValue,
    }),
