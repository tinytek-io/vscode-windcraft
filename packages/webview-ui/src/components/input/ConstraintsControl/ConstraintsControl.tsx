...
export type ConstraintsProps = VerticalConstraintsState &
  ...
  return (
    <div className="constraints-container flex border rounded aspect-square">
      ...
      <div className="flex flex-col justify-center p-px">
        {/* left */}
        <HorizontalButton
          selected={left.current || left.applied}
          applied={left.applied}
          onClick={() => onHorizontalClick("left")}
        />
      </div>
      <div className="flex flex-col w-full">
        ...
        <div className="flex justify-center p-px">
          {/* top */}
          <VerticalButton
            selected={top.current || top.applied}
            applied={top.applied}
            onClick={() => onVerticalClick("top")}
          />
        </div>
        <div className="constraints-center-box flex border rounded relative w-10 h-10" style={{ position: "relative" }}>
          {/* center box */}
        ...
        <div className="flex justify-center p-px">
          {/* bottom */}
          <VerticalButton
            selected={bottom.current || bottom.applied}
            applied={bottom.applied}
            onClick={() => onVerticalClick("bottom")}
          />
        </div>
      </div>
      <div className="flex flex-col justify-center p-px">
        {/* right */}
        <HorizontalButton
          selected={right.current || right.applied}
          applied={right.applied}
          onClick={() => onHorizontalClick("right")}
        />
      </div>
    </div>
  );
