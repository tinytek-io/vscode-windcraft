...
export function DynamicSection() {
  const { hasExactValue, updateCurrentStyles, dispatch, styleState } =
    useExtensionState();
  ...
  const deviceMode =
    modifierStates.find((mod) => deviceModes.includes(mod)) || "xs";
  const themeMode = modifierStates.includes("dark") ? "dark" : "light";
  const elementEvent =
    modifierStates.find((mod) => elementEventModes.includes(mod)) || "none";
  const elementForm =
    modifierStates.find((mod) => elementFormModes.includes(mod)) || "none";
        ...
        <div className="flex justify-between px-2">
          {deviceModes.map((dm) => (
            <TextButton
              ...
              className="w-6"
              onClick={() =>
                setModifierState(dm, themeMode, elementEvent, elementForm)
              }
        ...
        <div className="flex px-2 justify-between">
          <div className="flex gap-1">
            <LuSunMedium
              ...
              onClick={() =>
                setModifierState(deviceMode, "light", elementEvent, elementForm)
              }
              title="Light mode"
            />
            <LuMoonStar
              ...
              onClick={() =>
                setModifierState(deviceMode, "dark", elementEvent, elementForm)
              }
              title="Dark mode"
            />
          </div>
